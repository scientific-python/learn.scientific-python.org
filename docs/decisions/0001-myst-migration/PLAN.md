...
1. `docs: add ADRs for MyST migration decisions`
   - Files: `docs/decisions/README.md` (new),
     ...
     `docs/decisions/0001-myst-migration/0004-consume-cookie-myst-build.md` (new),
...
| 2   | Cookie              | **Consume cookie's own MyST build**: keep the submodule and `make cookie` target                                                  | Cookie is MyST upstream and owns its build; `learn` overlays its output at `/development/`. See ADR 0004.                            |
...
2. `config: document deferred footer/quicklinks` _(deferred)_
   - Files: add a comment block at the bottom of `myst.yml` with
     ...
     numbers.
   - **Prerequisite — file these three follow-up issues before writing
     ...
     2. "MyST: gh-pages PR preview for fork contributors" (ADR 0007)
     3. "MyST: replace netlify-plugin-checklinks with lychee GH Actions"
        (ADR 0007 option 3)
...
1. `build: rewrite Makefile for MyST`
   ...
   - New targets:
     ...
     - `cookie` / `external` / `prepare` / `help` — keep (ADR 0004: `learn`
       consumes cookie's own MyST build; `help` is `.DEFAULT_GOAL`)
